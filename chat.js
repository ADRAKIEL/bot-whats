...
if (!process.argv[2]) {
  console.log(logSymbols.error, chalk.red('Contacto no especificado...'));
  process.exit(1);
}
  ...
  try {
    ...
    async function readLastOtherPersonMessage() {
      ...
      message = await page.evaluate((selector) => {
        ...
        // check if it is picture message it was commented
	/*
	...
	*/
      ...
      if (message) {
		...
		if(message.toLowerCase() == "hola bot") {
			typeMessage("Hola " + name + " como estas!.");
		}
